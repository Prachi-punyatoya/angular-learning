...
app.controller('EditorCtrl', function($scope, filterFilter, $http, $location,  $timeout, $filter, $rootScope){
	...
	//  url: '/v2/config.php'
	  ...
	  params: d
		...
		/* to remove */
		//$scope.companyName = "Antoine";
		//$scope.id_categorie = 1000;
		//$scope.goEtape2(1000);
		/* end */
	...
	$scope.$watch('selectedItems', function(newValue, oldValue) {
		/*$timeout(function () {*/
			...
			{
				//console.log('set alignement');
				$scope.showBar('alignement');
			}*/
		});
	/*},0);*/
	/* END WATCH */
	...
	$scope.generateConfigs = function(icons, font, hideShapes)
	{
		...
		//ng-show="(config.type == 'shape' && filters.shapes == true) ||(config.type == 'conf' && filters.texts == true) || 
	...
	$scope.loadIcon = function (icon, etape)
	{
		...
		$timeout(function(){
			...
			$scope.favoris = [];
			config.shape.favoris = 1;
			
			angular.forEach($scope.configs, function(config, key) {
				
				if(config.shape.favoris == true){
					
					//var html = config.html.replace(/g\-/g, 'fav-g-');
					//var fav = {'html':html, 'config': config};
	...
	$scope.changeCategorie = function(categorie, force, onlyicon)
	{
		...
		if(categorie != $scope.lastCategorie || force == true){
			...
			var s = $scope.companyName+ " " + $scope.phraseAccroche;
	...
	$scope.resizeShape = function(e, ui){
		...
		if($scope.activeShape){
			...
			//$scope.$apply(function () {
					$scope.activeShape.tmp.ratio = ratioW;

					$scope.activeShape.tmp.width =  ui.size.width  - 2 * $scope.padding;

					
					if($scope.activeShape.aspectRatio === true){
						$scope.activeShape.tmp.height =  $scope.activeShape.originalHeight * ratioW;
					//	$scope.activeShape.height =  $scope.activeShape.originalHeight * ratioW;
						//$scope.activeShape.height = ui.size.height  - 2 * $scope.padding;
					}
					else 
					{
						//console.log("ui.size", ui.size.height);
						$scope.activeShape.tmp.height = ui.size.height  - 2 * $scope.padding;
					//	$scope.activeShape.height = ui.size.height  - 2 * $scope.padding;
					}
					
					var ratioH = $scope.activeShape.tmp.height /  $scope.activeShape.originalHeight;
					
					ui.position.left = $scope.activeShape.tmp.x - ($scope.activeShape.tmp.width - $scope.activeShape.originalWidth) / 2 - $scope.padding;
					ui.position.top = $scope.activeShape.tmp.y - ($scope.activeShape.tmp.height - $scope.activeShape.originalHeight) / 2 - $scope.padding;
					...
					{
						$('#shape-' + shape.id).attr('transform', 'translate(' + (shape.symetrieX == 1 ? left : left + shape.tmp_width)  +","+ (shape.symetrieY == 1 ? top : top + shape.tmp_height) + ") scale("+ratioW * shape.symetrieX + " "+ ratioH * shape.symetrieY + ')  rotate(' + (shape.rotate * shape.symetrieX * shape.symetrieY) + ' ' + (shape.originalWidth/2) + ' ' + (shape.originalHeight / 2)+")");
					}
					ui.size.height = $scope.activeShape.tmp.height + 2 * $scope.padding;
	...
	*/
	...
	$scope.backStep = function(){
		...
		if($scope.etape > 1){
			...
			$timeout(function(){
				//$('.logo-presentation').trigger("reload");
				$('.search-bar-container-fixed').css('width', "auto");
				var w = $('.sticky-wrapper').width();
				$('html, body').animate({scrollTop: $scope.scrollTop}, 0);
				$('.search-bar-container-fixed').css('width', w+ "px");
				$('.ng-presentation').trigger('reload');
	...
	*/
	...
	$scope.centerOnStage = function(shapes)
	{
		...
		//console.log("peillon",  $scope.id_logo);
		var left = 74;
		var top = 50;
	...
	$scope.generateSVG = function(){
		...
		var width = Math.ceil(svgCanvas[0].getBoundingClientRect().width)+3;
		...
		//var shapes = angular.copy($scope.shapes);
		
		/*var svg = '';
		var shapes = $scope.logo.shapes;
		minX = Infinity;
		minY = Infinity;
		maxX = 0;
		maxY = 0;
		...
		*/
		...
		$timeout(function(){
			...
			//var reg2=/<g id="svg-engine">([\x00-\xFF]+)<\/g>/gm;
	...
	$scope.resizeStiky = function(){
					/* resize pour la search bar */
			$('.search-bar-container-fixed').css('width', "auto");
			var w = $('.sticky-wrapper').width();
			$('.search-bar-container-fixed').css('width', w+ "px");
	}
